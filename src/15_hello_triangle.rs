...
impl HelloTriangleApplication {
    ...
    unsafe fn create_graphics_pipeline(
        ...
        let (descriptor_set_layouts, pipeline_layout, gfx_pipeline) = {
            ...
            let descriptor_set_layouts: Vec<<back::Backend as Backend>::DescriptorSetLayout> =
                vec![device.create_descriptor_set_layout(bindings, immutable_samplers).unwrap()];
            let push_constants = Vec::<(pso::ShaderStageFlags, std::ops::Range<u32>)>::new();
            let layout = device.create_pipeline_layout(&descriptor_set_layouts, push_constants).unwrap();
    ...
    unsafe fn create_command_pool(
        ...
        let raw_command_pool =
            device.create_command_pool(qf_id, pool::CommandPoolCreateFlags::empty()).unwrap();
